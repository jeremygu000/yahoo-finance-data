<div align="center">

# 📈 Market Terminal

**Real-time financial data pipeline with interactive dashboard**

[![CI](https://github.com/jeremygu000/yahoo-finance-data/actions/workflows/ci.yml/badge.svg)](https://github.com/jeremygu000/yahoo-finance-data/actions/workflows/ci.yml)
[![Python 3.12+](https://img.shields.io/badge/python-3.12%2B-blue.svg)](https://www.python.org/downloads/)
[![License: MIT](https://img.shields.io/badge/License-MIT-green.svg)](LICENSE)
[![Code style: black](https://img.shields.io/badge/code%20style-black-000000.svg)](https://github.com/psf/black)
[![mypy: strict](https://img.shields.io/badge/mypy-strict-blue.svg)](https://mypy-lang.org/)

Multi-source market data fetcher → Parquet storage → DuckDB analytics → REST & WebSocket API → Next.js dashboard with live charts, AI insights, and portfolio tracking.

</div>

![Dashboard Preview](docs/dashboard-preview.png)

## Features

- **Multi-source data** — Yahoo Finance, Tiingo, FMP with automatic fallback chain
- **Multi-timeframe** — 1m, 5m, 15m, 1h, 1d, 1wk, 1mo intervals
- **Real-time WebSocket** — Live price updates pushed to the dashboard
- **Price alerts** — Configurable above/below/percent-change triggers with WebSocket notifications
- **Alert notifications** — Telegram and Email (SMTP) channels for triggered alerts
- **Watchlist** — Persistent user-defined ticker lists
- **Interactive charts** — Candlestick (K-line), multi-ticker comparison, VIX dashboard, indicator overlays
- **Historical backfill** — CLI command to fetch arbitrary date ranges
- **10 technical indicators** — SMA, EMA, RSI, MACD, Bollinger Bands, VWAP, ATR, Stochastic, OBV, ADX
- **Company fundamentals** — P/E, EPS, dividends, market cap, sector, 52-week range via yfinance
- **News feed** — Per-ticker news articles with thumbnails from Yahoo Finance
- **AI market analysis** — Ollama-powered summaries, streaming responses, and multi-turn chat
- **Market heatmap** — Visual overview of ticker performance
- **Portfolio tracking** — Holdings management with real-time P&L summary
- **Data quality monitoring** — Anomaly detection, NaN density, staleness, gap analysis via DuckDB
- **DuckDB analytics** — High-performance batch queries across all Parquet files
- **CSV export** — Download OHLCV data as CSV files
- **Ticker search** — Autocomplete search across cached tickers
- **Command palette** — Cmd+K quick navigation
- **Smart caching** — In-memory LRU cache with configurable TTL
- **Error recovery** — Exponential backoff with jitter for failed provider requests
- **API authentication** — Optional API key protection (`MARKET_DATA_API_KEY`)
- **Scheduled fetching** — macOS launchd integration for automatic daily updates
- **Type-safe** — mypy strict, 340+ tests, full Pydantic schemas

## Architecture

```
┌─────────────────┐     ┌───────────────────┐     ┌─────────────────┐
│   Data Sources  │     │   Storage Layer   │     │   Consumers     │
│                 │     │                   │     │                 │
│  Yahoo Finance  │────▶│  Parquet Files    │────▶│  Python API     │
│  Tiingo         │     │  (~/.market_data) │     │  FastAPI REST   │
│  FMP            │     │                   │     │  WebSocket /ws  │
│                 │     │  DuckDB (read)    │     │  Next.js UI     │
└─────────────────┘     └───────────────────┘     │  Ollama AI      │
                                                  └─────────────────┘
```

## Quick Start

### Docker (recommended)

```bash
cp .env.example .env    # configure API keys (optional — yfinance works without keys)

docker compose up -d                         # start backend + frontend + nginx
docker compose run --rm fetch                # fetch initial data (one-time)

# Dashboard at http://localhost (nginx reverse proxy)
# API docs at http://localhost/docs
```

### Local Development

```bash
# Backend
uv sync
uv run market-data fetch           # fetch data
uv run uvicorn market_data.server:app --port 8100

# Frontend
cd web && pnpm install && pnpm dev

# Dashboard at http://localhost:3000
```

## CLI

```bash
market-data fetch                           # incremental fetch (rolling 30-day window)
market-data fetch --full                    # full 1-year historical fetch
market-data fetch --tickers AAPL,MSFT       # custom tickers
market-data backfill --ticker AAPL \
    --start 2020-01-01 --end 2023-12-31     # historical backfill
market-data status                          # show cached data summary
market-data clean                           # remove data older than 365 days
market-data clean --keep-days 180           # custom retention

market-data watchlist list                  # show watchlist
market-data watchlist add AAPL              # add ticker
market-data watchlist remove AAPL           # remove ticker

market-data alerts list                     # show all alerts
market-data alerts add AAPL above 200       # trigger when AAPL > $200
market-data alerts add QQQ percent_change_below -5  # trigger on -5% drop
market-data alerts remove <alert-id>        # delete alert

market-data portfolio list                  # show holdings
market-data portfolio add AAPL --shares 10 --cost 150.0  # add holding
market-data portfolio remove AAPL           # remove holding
```

## API

All endpoints are available under `/api/v1/` (and legacy `/api/` for backward compatibility).

### Market Data

| Method | Endpoint | Description |
|--------|----------|-------------|
| `GET` | `/api/v1/tickers` | List all cached tickers with metadata |
| `GET` | `/api/v1/tickers/names` | List ticker symbols only |
| `GET` | `/api/v1/tickers/overview` | Ticker overview with live prices |
| `GET` | `/api/v1/heatmap` | Market heatmap data |
| `GET` | `/api/v1/ohlcv/{ticker}?days=365&interval=1d` | OHLCV bars with pagination |
| `GET` | `/api/v1/latest/{ticker}` | Latest quote for a ticker |
| `GET` | `/api/v1/compare?tickers=QQQ,XOM&days=90` | Multi-ticker close price comparison |
| `GET` | `/api/v1/indicators/{ticker}?indicator=sma&period=20` | Technical indicators (10 types) |
| `GET` | `/api/v1/fundamentals/{ticker}` | Company fundamentals (P/E, EPS, etc.) |
| `GET` | `/api/v1/news/{ticker}?count=10` | News articles with thumbnails |
| `GET` | `/api/v1/export/{ticker}?format=csv&days=365` | Download OHLCV data as CSV |
| `GET` | `/api/v1/search?q=AA&limit=10` | Search cached tickers |

### Watchlist & Alerts

| Method | Endpoint | Description |
|--------|----------|-------------|
| `GET` | `/api/v1/watchlist` | Get watchlist |
| `POST` | `/api/v1/watchlist` | Add ticker to watchlist |
| `DELETE` | `/api/v1/watchlist/{ticker}` | Remove ticker from watchlist |
| `GET` | `/api/v1/alerts` | List price alerts |
| `POST` | `/api/v1/alerts` | Create price alert |
| `DELETE` | `/api/v1/alerts/{alert_id}` | Delete price alert |
| `GET` | `/api/v1/alerts/channels` | List notification channels (Telegram, Email) |
| `POST` | `/api/v1/alerts/test/{alert_id}` | Send test notification for an alert |

### Portfolio

| Method | Endpoint | Description |
|--------|----------|-------------|
| `GET` | `/api/v1/portfolio` | List portfolio holdings |
| `POST` | `/api/v1/portfolio` | Add holding (ticker, shares, avg_cost) |
| `PUT` | `/api/v1/portfolio/{ticker}` | Update holding |
| `DELETE` | `/api/v1/portfolio/{ticker}` | Remove holding |
| `GET` | `/api/v1/portfolio/summary` | Portfolio with live P&L |

### AI Analysis (Ollama)

| Method | Endpoint | Description |
|--------|----------|-------------|
| `GET` | `/api/v1/ai/health` | Check Ollama connectivity |
| `POST` | `/api/v1/ai/summary` | Generate market summary for tickers |
| `POST` | `/api/v1/ai/summary/stream` | Streaming market summary (SSE) |
| `POST` | `/api/v1/ai/chat` | Multi-turn AI chat about market data |

### System

| Method | Endpoint | Description |
|--------|----------|-------------|
| `WS` | `/ws/prices` | Real-time price updates + alert notifications |
| `GET` | `/health` | Health check |
| `GET` | `/ready` | Readiness check (data availability) |
| `GET` | `/docs` | Interactive API documentation (Swagger UI) |

## Python API

Use as a library from any local Python project:

```python
from market_data import get_ohlcv, get_latest, list_tickers

df = get_ohlcv("QQQ", days=30)       # pandas DataFrame
latest = get_latest("CRM")            # dict with latest OHLCV
tickers = list_tickers()              # all cached tickers
```

## Web Dashboard

| Section | Description |
|---------|-------------|
| **Hero Banner** | Market summary header with key indices |
| **Ticker Overview** | Live price cards with daily change, volume, and LIVE indicator |
| **Candlestick Chart** | K-line with volume bars (TradingView Lightweight Charts) |
| **Indicator Chart** | Technical indicator overlays (SMA, RSI, MACD, etc.) |
| **Price Comparison** | Multi-ticker overlay line chart |
| **Data Table** | Sortable OHLCV table with pagination |
| **VIX Dashboard** | Current VIX level, zone gauge, historical chart |
| **Fundamentals Card** | Company overview — sector, P/E, EPS, dividends, market cap |
| **News Feed** | Per-ticker news articles with thumbnails and article modal |
| **AI Summary** | Ollama-powered market analysis and interactive chat |
| **Portfolio Dashboard** | Holdings with real-time P&L, add/remove positions |
| **Alert Manager** | Create/manage price alerts with toast notifications |
| **Command Palette** | Cmd+K quick navigation and ticker search |

## Configuration

Copy `.env.example` to `.env` and customize:

### Core

| Variable | Default | Description |
|----------|---------|-------------|
| `MARKET_DATA_PROVIDER` | `yfinance` | Primary data provider |
| `MARKET_DATA_FALLBACK_CHAIN` | `yfinance,tiingo,fmp` | Provider fallback order |
| `TIINGO_API_KEY` | — | Tiingo API key |
| `FMP_API_KEY` | — | Financial Modeling Prep API key |
| `MARKET_DATA_DIR` | `~/.market_data/parquet` | Parquet storage path |
| `MARKET_DATA_LOOKBACK_DAYS` | `365` | Default historical lookback |
| `MARKET_DATA_BATCH_SIZE` | `50` | Concurrent fetch batch size |
| `MARKET_DATA_FETCH_CONCURRENCY` | `4` | Parallel fetch workers |
| `MARKET_DATA_CORS_ORIGINS` | `http://localhost:3000` | Allowed CORS origins |
| `MARKET_DATA_API_KEY` | — | API key for authentication (optional) |
| `MARKET_DATA_MAX_RETRIES` | `3` | Max retry attempts for failed provider requests |

### WebSocket & Cache

| Variable | Default | Description |
|----------|---------|-------------|
| `MARKET_DATA_WS_POLL_INTERVAL` | `5` | WebSocket price poll interval (seconds) |
| `MARKET_DATA_WS_HEARTBEAT_INTERVAL` | `15` | WebSocket heartbeat interval (seconds) |
| `MARKET_DATA_CACHE_TTL` | `120` | In-memory cache TTL (seconds) |
| `MARKET_DATA_CACHE_MAX_ENTRIES` | `4096` | Maximum cache entries |

### Notifications

| Variable | Default | Description |
|----------|---------|-------------|
| `TELEGRAM_BOT_TOKEN` | — | Telegram bot token for alert notifications |
| `TELEGRAM_DEFAULT_CHAT_ID` | — | Default Telegram chat ID |
| `SMTP_HOST` | `smtp.gmail.com` | SMTP server hostname |
| `SMTP_PORT` | `587` | SMTP server port |
| `SMTP_USERNAME` | — | SMTP username |
| `SMTP_PASSWORD` | — | SMTP password |
| `SMTP_FROM` | — | Sender email address |

### AI (Ollama)

| Variable | Default | Description |
|----------|---------|-------------|
| `OLLAMA_HOST` | `http://localhost:11434` | Ollama server URL |
| `OLLAMA_MODEL` | `qwen3:32b` | Ollama model for market analysis |
| `OLLAMA_TIMEOUT` | `180` | Ollama request timeout (seconds) |

## Scheduled Fetch (macOS)

Auto-fetch Mon–Fri after US market close:

```bash
bash install_schedule.sh
```

Installs a launchd plist that runs `market-data fetch` at 20:35 UTC (4:35 PM ET).

## Project Structure

```
market-terminal/
├── src/market_data/
│   ├── __init__.py          # public API re-exports
│   ├── ai_summary.py        # Ollama AI summaries, streaming, multi-turn chat
│   ├── alerts.py            # price alert engine
│   ├── api.py               # get_ohlcv, get_latest, list_tickers
│   ├── cache.py             # LRU cache with TTL
│   ├── cli.py               # CLI entry point (fetch, backfill, watchlist, alerts, portfolio)
│   ├── config.py            # env-configurable constants
│   ├── duckdb_reader.py     # DuckDB batch Parquet reader (heatmap, overview, quality)
│   ├── exceptions.py        # custom exception hierarchy
│   ├── fetcher.py           # multi-provider fetch with fallback + retry
│   ├── fundamentals.py      # company fundamentals (P/E, EPS, sector, dividends)
│   ├── indicators.py        # 10 technical indicators (SMA, EMA, RSI, MACD, BB, VWAP, ATR, Stoch, OBV, ADX)
│   ├── logging_config.py    # structured logging setup
│   ├── news.py              # ticker news feed from Yahoo Finance
│   ├── notifications.py     # alert channels (Telegram, Email/SMTP)
│   ├── portfolio.py         # portfolio holdings (JSON persistence)
│   ├── quality.py           # data quality checks (anomalies, NaN, staleness, gaps)
│   ├── schemas.py           # Pydantic request/response models
│   ├── server.py            # FastAPI app + WebSocket + CORS + 29 endpoints
│   ├── store.py             # Parquet read/write with interval support
│   ├── watchlist.py         # persistent watchlist (JSON)
│   └── providers/
│       ├── base.py          # MarketDataProvider protocol
│       ├── yfinance.py      # Yahoo Finance provider
│       ├── tiingo.py        # Tiingo provider
│       └── fmp.py           # FMP provider
├── web/                     # Next.js 16 dashboard
│   └── src/
│       ├── app/             # App Router pages (home, portfolio)
│       ├── components/      # 20 components (charts, alerts, news, AI, etc.)
│       └── lib/             # API client, WebSocket hook, types
├── tests/                   # 340+ tests (pytest)
├── nginx/                   # nginx reverse proxy config
├── docker-compose.yml       # one-command deployment (nginx + backend + frontend)
├── Dockerfile               # backend container (multi-stage, uv)
├── web/Dockerfile           # frontend container
├── .github/workflows/       # CI (black, mypy, pytest, pnpm lint+typecheck+build, pip-audit, Docker)
├── CONTRIBUTING.md          # contribution guide
├── CHANGELOG.md             # release history
└── pyproject.toml
```

## Development

```bash
uv run pytest -v                        # run tests
uv run pytest --cov=market_data         # with coverage
uv run mypy src/market_data/            # type check
uv run black src/ tests/                # format

cd web && pnpm lint && pnpm build       # frontend checks
```

## Requirements

- Python 3.12+
- Node.js 22+ / pnpm (for web dashboard)
- [uv](https://docs.astral.sh/uv/) (Python package manager)
- Docker (optional, for containerized deployment)
- [Ollama](https://ollama.ai/) (optional, for AI analysis features)

## License

[MIT](LICENSE) © Jeremy Gu
